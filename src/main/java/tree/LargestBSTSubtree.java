 ...
 * 
 * Given a binary tree, find the largest subtree which is a BST, where
 * largest means subtree with largest number of nodes in it.
 * 
 * NOTE: Subtree must include all its descendants
 */ 
...
public class LargestBSTSubtree {
    ...
    private int largestBSTSubtree(TreeNode root, int minValue, int maxValue) {
        ...
        if (root.val < minValue || root.val > maxValue) return -1;
        if (ln == - 1 || rn == -1) return 1;
        int currNum = ln + rn + 1;
